...
启动多个Redis实例时，相互之间就可以通过`replicaof`命令<u>**形成主库和从库**</u>的关系。 例如现在有两个实例，分别是实例1（127.0.0.1)和实例2（127.0.0.2）,在实例2上执行如下命令就可成为实例1的从库，并且从实例1上复制数据。
...
为了保证主从库的数据一致性，主库会在内存中用专门的`replication buffer`，记录RDB文件生成后收到的所有写操作；
...
#### *上一个slave节点可以作为另一个slave节点的master*
...
但是主从库网络断连或者阻塞，主从库之间无法进行命令传播，那么从库的数据也没法和主库保持一致。
...
## # [(126条消息) Redis(五）--高可用主从复制详解\_想当程序媛的秀妍的博客-CSDN博客](https://blog.csdn.net/m0_52675592/article/details/116536969)

> [(126条消息) Redis(五）--高可用主从复制详解\_想当程序媛的秀妍的博客-CSDN博客](https://blog.csdn.net/m0_52675592/article/details/116536969)
>
> 如果主从库在命令传播时出现了**网络闪断**，那么，从库就会和主库重新进行一次全量复制，开销非常大。从Redis2.8开始，网络断了之后，主从库会采用增量复制的方式继续同步。
>
> 如果在网络断开期间， repl_backlog_size环形缓冲区写满之后，从库是会丢失掉那部分被覆盖掉的数据，还是会直接进行全量复制呢？
>
> - 一个从库如果和主库断连时间过长，造成它在主库repl_backlog_buffer的slave_repl_offset位置上的数据已经被覆盖掉了，此时从库和主库间将进行全量复制。
>
> - 每个从库会记录自己的slave_repl_offset，每个从库的复制进度也不一定相同。在和主库重连进行恢复时，从库会通过psync命令把自己记录的slave_repl_offset发给主库，主库会根据从库各自的复制进度，来决定这个从库可以进行增量复制还是全量复制。